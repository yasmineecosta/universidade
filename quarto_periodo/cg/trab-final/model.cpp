...
GLuint width = 920, height = 740; // Largura e altura da janela
...
enum animation_Boy_ids{idle, running, jumping, shooting};
enum animation_Girl_ids{idle, running, jumping, shooting};
int animation_idBoy = idle; //Indica a animacao a ser desenhada atualmente
int animation_idGirl = idle; //Indica a animacao a ser desenhada atualmente
int current_animationBoy = idle; // Salva qual e a animacao atual
int current_animationGirl = idle; // Salva qual e a animacao atual
bool is_jumpingBoy = false, is_shootingBoy = false;
bool is_jumpingGirl = false, is_shootingGirl = false;

GLManimation * animationBoy = NULL;   //Ponteiro usado no armazenamento de uma animacao
GLManimation * animationGirl = NULL;  //Ponteiro usado no armazenamento de uma animacao
map <int, GLManimation *> animationsBoy, animationsGirl; //Mapeamento dos identificadores com as animacoes
...
float scale = 3; // fator de escala proporcional em todos os eixos do modelo 3D
float pos = 1.0; // posicao da bola em cada frame
...
//void drawAnimation(int type, GLuint mode, CharacterState& state, std::map<int, GLManimation*>& animations);
void drawFloor(GLuint mode);
void playAnimation(int id);
void computeFPS(int keyframe_rate);
//void playAnimation(int id, CharacterState& state);
void timer(int value);
void keyPlayAnimation(int id);
//void keyPlayAnimation(int id, CharacterState& state);
...
//    setlocale(LC_ALL,"Portuguese");
    ...
    init_glut("FireBoy e WaterGirl: elementos na dungeon", argc, argv);
...
void init_glut(const char *nome_janela, int argc, char** argv){
    ...
    */ 
    ...
    animationBoy = glmLoadAnimation("models/Boy/animations/idle", idle, 500);
    animationGirl = glmLoadAnimation("models/Girl/animations/idle", idle, 301);
    faces = animationBoy->models[0]->numtriangles;
    vertices = animationBoy->models[0]->numvertices;
    animationBoy->name = "idle";
    animationGirl->name = "idle";
    animationBoy[idle] = animationBoy;
    animationGirl[idle] = animationGirl;
    printf(".");
    animationBoy = glmLoadAnimation("models/Boy/animations/jumping", jumping, 58);
    animationGirl = glmLoadAnimation("models/Girl/animations/jumping", jumping, 58);
    animationsBoy[jumping] = animationBoy;
    printf(".");
    animationBoy = glmLoadAnimation("models/Boy/animations/running", running, 22);
    animationGirl = glmLoadAnimation("models/Girl/animations/running", running, 22);
    animationsBoy[running] = animationBoy;
    printf(".");
    animationBoy = glmLoadAnimation("models/Boy/animations/shooting", shooting, 77);
    animationGirl = glmLoadAnimation("models/Girl/animations/shooting", shooting, 77);
    animationsBoy[shooting] = animationBoy;
    printf(" done.\n");
    /*
    // Carregar animacoes para Boy
    boy_animations[paused] = glmLoadAnimation("models/Boy/animations/idle/idle1.obj", paused, 1);
    boy_animations[idle] = glmLoadAnimation("models/Boy/animations/idle", idle, 500);
    boy_animations[jumping] = glmLoadAnimation("models/Boy/animations/jumping", jumping, 58);
    boy_animations[running] = glmLoadAnimation("models/Boy/animations/running", running, 22);
    boy_animations[shooting] = glmLoadAnimation("models/Boy/animations/shooting", shooting, 77);

    // Carregar animacoes para Girl
    girl_animations[paused] = glmLoadAnimation("models/Girl/animations/idle/idle1.obj", paused, 1);
    girl_animations[idle] = glmLoadAnimation("models/Girl/animations/idle", idle, 500);
    girl_animations[jumping] = glmLoadAnimation("models/Girl/animations/jumping", jumping, 58);
    girl_animations[running] = glmLoadAnimation("models/Girl/animations/running", running, 22);
    girl_animations[shooting] = glmLoadAnimation("models/Girl/animations/shooting", shooting, 77);

    faces = boy_animations[paused]->models[0]->numtriangles;
    vertices = boy_animations[paused]->models[0]->numvertices;
...
void display(void){
    ...
    glPushMatrix();
        ...
        drawAnimation(animation_idBoy, animation_idGirl, mode);
        
        /*drawAnimation(boy_state.animation_id, mode, boy_state, boy_animations, boy_scale);
    */
    glPopMatrix();
    ...
    /*
    glPushMatrix();
    drawAnimation(girl_state.animation_id, mode, girl_state, girl_animations, girl_scale);
    glPopMatrix();
	*/
    drawFloor(mode); //Desenha o chao
    
    if(is_jumpingBoy && keyframe == animationsBoy[jumping]->keyframes - 1) is_jumpingBoy = false;
    
    if(animation_idBoy == shooting && keyframe >= 30){
		glPushMatrix();
		glTranslatef(-0.4, 0.3, pos);
		glutSolidSphere(0.5, 50, 50);
		glPopMatrix();
		pos+=0.5;
	}
	/*
	if (boy_state.is_jumping && keyframe == boy_animations[jumping]->keyframes - 1) boy_state.is_jumping = false;
    if (girl_state.is_jumping && keyframe == girl_animations[jumping]->keyframes - 1) girl_state.is_jumping = false;

    if (boy_state.animation_id == shooting && keyframe >= 30) {
        glPushMatrix();
        glTranslatef(-0.4, 0.3, pos);
        glutSolidSphere(0.5, 50, 50);
        glPopMatrix();
    ...
    */
    ...
    draw_text_stroke(x , 1.15,"(R)RESET (W/S)ROTX (A/D)ROTY (Arrows)RUN (Space)JUMP (F/f)Shooting (F1)PROJ: " + pro );
...
void computeFPS(int keyframe_rate){
    ...
    if(!is_paused){ // Verifica se a animacao esta em pausa
        ...
        /*
        int size = boy_animations[boy_state.current_animation]->keyframes; // Quantidade de frames da animacao atual

        //Se animacao == idle, depois de reproduzir 5 vezes a animacao idle reproduz a animacao bored
        if (boy_state.animation_id == idle && frames_playing >= (size * keyframe_rate * 5) + 1) playAnimation(bored, boy_state);

        //Verifica se foi alterada a animacao atual e reinicia os contadores para a nova animacao
        else if (boy_state.current_animation != boy_state.animation_id) playAnimation(boy_state.animation_id, boy_state);

        //Se uma animacao diferente de idle ou bored nao for mais reiniciada, reproduz a animacao idle
        else if (boy_state.animation_id != idle && boy_state.animation_id != bored && keyframe >= size && frames_playing >= size) playAnimation(idle, boy_state);
 ...
 * Desenha a animacao
 *//*
void drawAnimation(int id, GLuint mode, CharacterState& state, std::map<int, GLManimation*>& animations, float scale) {
    glPushMatrix();
    glScalef(scale, scale, scale);
    glTranslatef(state.posx, state.posy, state.posz);
    glRotatef(state.rotx, 1.0f, 0.0f, 0.0f);
    glRotatef(state.roty, 0.0f, 1.0f, 0.0f);
    glRotatef(state.rotz, 0.0f, 0.0f, 1.0f);
    glmDrawAnimation(animations[id], keyframe, mode);
    glPopMatrix();
}
*/
...
void keyPlayAnimation(int id){
    frames_playing = 0;
    animation_idBoy = id;
    if(id == jumping) is_jumpingBoy = true;
    if(id == shooting) is_shootingBoy = true;
}
/*
void keyPlayAnimation(int id, CharacterState& state) {
    frames_playing = 0;
    state.animation_id = id;
    if (id == jumping) state.is_jumping = true;
    if (id == shooting) state.is_shooting = true;
}
*/
/*
...
//        case '+': case '=': if(graus < 360) graus += 1; break;
    	...
    	/*
		case SPACE:
	        if (!is_paused) keyPlayAnimation(jumping, boy_state);
	        break; // Tecle SPACE para ativar a animacao de salto
	    case 'p':
	    case 'P':
	        is_paused = !is_paused;
	        boy_state.animation_id = paused;
	        keyframe = 0;
	        break;
	    case 'f':
	    case 'F':
	        if (!is_paused) keyPlayAnimation(shooting, boy_state);
	        break;
	    case 'd':
	    case 'D':
	        // Move boy to the right
	        boy_state.roty -= graus;
	        if (boy_state.roty < 0) boy_state.roty += 360;
	        update_direcao(boy_state);
	        break;
	    case 'a':
	    case 'A':
	        // Move boy to the left
	        boy_state.roty += graus;
	        if (boy_state.roty >= 360) boy_state.roty -= 360;
	        update_direcao(boy_state);
	        break;
	
	    // Girl controls
	    case ENTER:
	        if (!is_paused) keyPlayAnimation(jumping, girl_state);
	        break; // Tecle ENTER para ativar a animacao de salto
	    case BACKSPACE:
	        if (!is_paused) keyPlayAnimation(shooting, girl_state);
	        break;
	
	    // Tecle < ou > para alterar o FPS desejado
	    case ',':
	    case '<':
	        if (fps_desejado > 1) fps_desejado -= 1;
	        break;
	    case '.':
	    case '>':
	        if (fps_desejado * 2 < MAX_FPS) fps_desejado += 1;
	        break;
	    case ESC:
	        exit(EXIT_SUCCESS);
	        break; // Tecle ESC para sair do programa
			*/
	}
}
...
//                break;
        ...
        case GLUT_KEY_RIGHT: // Move o personagem para direita
            if(!is_paused){
                if(is_jumpingBoy) keyPlayAnimation(jumping);
                else if(is_shootingBoy) keyPlayAnimation(shooting);
        ...
        /*
        //Use as setas do teclado para movimentar a Girl
    case GLUT_KEY_RIGHT: // Move Girl para direita
        girl_state.roty -= graus;
        if (girl_state.roty < 0) girl_state.roty += 360;
        update_direcao(girl_state);
        break;
    case GLUT_KEY_LEFT: // Move Girl para esquerda
        girl_state.roty += graus;
        if (girl_state.roty >= 360) girl_state.roty -= 360;
        update_direcao(girl_state);
        break;
		*/
    }
...
void update_direcao(CharacterState& state) {
    if (state.roty == 0.0 || state.roty == 360.0) {
        state.direcao = frente;
        state.roty = 0.0;
    }
    if (state.roty == 180.0 || state.roty == -180.0) {
        state.direcao = tras;
        state.roty = 180.0;
    }
    if (state.roty == 90.0) state.direcao = direita;
    if (state.roty == -90.0 || state.roty == 270.0) state.direcao = esquerda;
}
*/
/*
