...
enum animation_ids{paused, idle, running, bored, jumping, shooting};
...
bool is_jumping = false, is_shooting = false;
...
//void drawAnimation(int type, GLuint mode, CharacterState& state, std::map<int, GLManimation*>& animations, float scale);
...
void init_glut(const char *nome_janela, int argc, char** argv){
    ...
    */ 
    ...
    animation = glmLoadAnimation("models/Girl/animations/idle/idle1.obj", paused, 1);
    ...
    animation = glmLoadAnimation("models/Boy/animations/idle", idle, 301);
...
void display(void){
	...
	*/
    ...
    if(animation_id == shooting && keyframe >= 30){
		glPushMatrix();
