...
void buscarVariavelEntrada() {
  variavel_entrada = minimus ? 1000 : -1000;
  colunaPivo = 0;
...
void buscarLinhaPivo() {
  ...
  for (int i = 0; i < totalLinhas - 1; i++) {
    if (tabela[i][colunaPivo] > 0) {
      coluna_variavel_entrada =
          tabela[i][totalColunas - 1] / tabela[i][colunaPivo];
...
int main() {
  cout << "O caso eh de maximo(0) ou de minimo(1)?\n";
  cin >> minimus;
  cout << "O caso a ser colocado eh de: ";
  if (minimus)
    cout << "Minimo\n";
  else
    cout << "Maximo\n";
  ...
  for (int i = 0; i < totalLinhas; i++) {
    for (int j = 0; j <= qtdX; j++) {
      if (i == totalLinhas - 1) {
        if (j == qtdX)
          cout << "Informe o valor da variavel de Z na funcao objetivo(0) : ";
        else
          cout << "Informe o valor da variavel " << j + 1
               << " na funcao objetivo : ";
      } else {
        if (j == qtdX)
          cout << "Informe o valor da restricao " << i + 1 << ": ";
        else
          cout << "Informe o valor da restricao " << i + 1 << " e da variavel "
               << j + 1 << ": ";
      }
      cin >> tabela[i][j];
    }
  ...
  for (int i = 0; i < qtdFuncoesLimitantes; i++) {
    for (int j = qtdX; j < totalColunas - 1; j++)
      tabela[i][j] = 0;
  ...
  for (int i = qtdX; i < totalColunas; i++)
    tabela[qtdFuncoesLimitantes][i] = 0;
  ...
  cout << "Valor de Z (LUCRO MAXIMO OU GASTO MINIMO): ";
  if (minimus)
    cout << respostaFinal;
  else
    cout << -respostaFinal;
